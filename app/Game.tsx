...
export default function Game() {
  ...
  useEffect(() => {
    ...
    const state = {
      ...
      enemies: Array.from({ length: 43 }, (_, i) => {const level=Math.min(3,Math.floor((600+i*270)/LEVEL_LENGTH));const depth=420+(i%3)*55;return {x:600+i*270,y:depth-43,depth,hp:1+level+(i%7===6?1:0),cooldown:55+(i*17)%70,alive:true,facing:-1,variant:[0,1,2,3][i%4],squad:Math.floor(i/3),moving:false};}),
    };

    const animeAtlas=new Image();let atlasReady=false;animeAtlas.onload=()=>{atlasReady=true};animeAtlas.src="sprites/anime-atlas-v2.png";
    ...
    const drawHero = (x: number, y: number, facing: number) => {
      ...
      if(atlasReady){const index=HEROES.findIndex(h=>h.id===hero.id);const cellW=animeAtlas.width/4;const cellH=animeAtlas.height/2;const moving=Boolean(keys.current.KeyA||keys.current.KeyD||keys.current.KeyW||keys.current.KeyS||keys.current.ArrowLeft||keys.current.ArrowRight||keys.current.ArrowUp||keys.current.ArrowDown);const phase=performance.now()/82;const step=moving?Math.sin(phase):0;const bob=moving?Math.abs(step)*3:0;ctx.save();ctx.translate(x+(facing<0?44:0)+20,y+8-bob);ctx.rotate(step*.014);ctx.scale(facing*(1+Math.abs(step)*.015),1-Math.abs(step)*.012);ctx.drawImage(animeAtlas,index*cellW,0,cellW,cellH,-41,-55,94,108);ctx.restore();return;}
    ...
    const draw = () => {
      ...
      for(const e of state.enemies){if(!e.alive)continue;const x=e.x-state.camera;if(x>-80&&x<WIDTH+80){ctx.fillStyle="rgba(0,0,0,.3)";ctx.beginPath();ctx.ellipse(x+18,e.depth+3,25,7,0,0,Math.PI*2);ctx.fill();if(atlasReady){const cellW=animeAtlas.width/4;const cellH=animeAtlas.height/2;const phase=performance.now()/82+e.squad;const step=e.moving?Math.sin(phase):0;const bob=e.moving?Math.abs(step)*3:0;ctx.save();ctx.translate(x+(e.facing<0?38:0)+18,e.y+3-bob);ctx.rotate(step*.014);ctx.scale(e.facing*(1+Math.abs(step)*.015),1-Math.abs(step)*.012);ctx.drawImage(animeAtlas,e.variant*cellW,cellH,cellW,cellH,-36,-44,82,88);ctx.restore();}else{ctx.save();ctx.translate(x+(e.facing<0?34:0),e.y);ctx.scale(e.facing,1);ctx.fillStyle="#351e29";ctx.fillRect(0,0,34,43);ctx.fillStyle="#8cffcf";ctx.fillRect(5,8,7,4);ctx.fillRect(22,8,7,4);ctx.fillStyle="#bf4859";ctx.fillRect(28,19,21,7);ctx.restore();}}}
