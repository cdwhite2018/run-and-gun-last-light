...
type Hero = { id: HeroId; name: string; tag: string; color: string; accent: string };

const HEROES: Hero[] = [
  { id: "ninja", name: "Kage", tag: "Shadow Ninja", color: "#171a25", accent: "#f04464" },
  { id: "operator", name: "Rook", tag: "Special Ops", color: "#536348", accent: "#d5bb76" },
  { id: "skeleton", name: "Cinder", tag: "Demon Bones", color: "#37223e", accent: "#ff7438" },
  { id: "granny", name: "Mabel", tag: "Muumuu Mayhem", color: "#79528f", accent: "#ffcf5a" },
];
...
export default function Game() {
  ...
  useEffect(() => {
    ...
    const state = {
      ...
      enemies: Array.from({ length: 22 }, (_, i) => ({ x: 820 + i * 285, y: 407, hp: i % 6 === 5 ? 3 : 1, cooldown: 40 + (i * 17) % 80, alive: true })),
    };
    ...
    const drawHero = (x: number, y: number) => {
      ...
      if (hero.id === "ninja") { ctx.fillRect(4, 1, 30, 10); ctx.fillRect(28, 8, 20, 4); }
      if (hero.id === "operator") { ctx.fillRect(5, -2, 28, 9); ctx.fillStyle="#1a211a"; ctx.fillRect(7, 15, 24, 12); }
      if (hero.id === "skeleton") { ctx.beginPath(); ctx.arc(19, 5, 12, 0, Math.PI*2); ctx.fill(); ctx.fillStyle="#241627"; ctx.fillRect(12, 2, 4, 4); ctx.fillRect(23, 2, 4, 4); }
      if (hero.id === "granny") { ctx.beginPath(); ctx.moveTo(19, 8); ctx.lineTo(38, 38); ctx.lineTo(0, 38); ctx.closePath(); ctx.fill(); ctx.fillStyle="#e8b58c"; ctx.beginPath(); ctx.arc(19, 2, 11, 0, Math.PI*2); ctx.fill(); }
      ctx.fillStyle="#252632"; ctx.fillRect(10, 36, 7, 10); ctx.fillRect(25, 36, 7, 10);
      ctx.fillStyle="#c5c8d2"; ctx.fillRect(28, 17, 25, 6); ctx.fillStyle="#ffcf5a"; ctx.fillRect(51, 18, 7, 4);
      ctx.restore();
    };
    ...
    const draw = () => {
      ...
      for(const e of state.enemies){if(!e.alive)continue;const x=e.x-state.camera;if(x>-60&&x<WIDTH+60){ctx.fillStyle="#351e29";ctx.fillRect(x,e.y,34,43);ctx.fillStyle="#8cffcf";ctx.fillRect(x+5,e.y+8,7,4);ctx.fillRect(x+22,e.y+8,7,4);ctx.fillStyle="#bf4859";ctx.fillRect(x-15,e.y+19,19,6);}}
      ...
      drawHero(state.player.x-state.camera,state.player.y);
    ...
    const loop = (now: number) => {
      ...
      for(const e of state.enemies){if(!e.alive)continue;e.cooldown-=dt;if(Math.abs(e.x-p.x)<520&&e.cooldown<=0){state.bullets.push({x:e.x-10,y:e.y+21,vx:-7,enemy:true});e.cooldown=90+Math.random()*60;}}
      for(const b of state.bullets){b.x+=b.vx*dt;if(b.enemy&&rectHit({x:b.x,y:b.y,w:9,h:4},{x:p.x,y:p.y,w:40,h:48})&&p.invuln<=0){p.hp-=10;p.invuln=35;b.x=-999;burst(p.x,p.y+20,"#ff6579");}if(!b.enemy)for(const e of state.enemies){if(e.alive&&rectHit({x:b.x,y:b.y,w:14,h:4},{x:e.x,y:e.y,w:34,h:43})){e.hp--;b.x=99999;burst(e.x,e.y+18,"#ffb13b");if(e.hp<=0){e.alive=false;state.score+=100;}}}}
